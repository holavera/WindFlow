...
class EstadisticasGrupoView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for perfil in usuarios:
            ...
            valores.append(completadas.count())
...
class ExportarEstadisticasPDFView(View):
    def get(self, request, grupo_id):
        ...
        # Hoy y ayer como cadenas
        hoy = localtime().date()
        ayer = hoy - timedelta(days=1)

        dias_validos = [hoy.strftime("%Y-%m-%d"), ayer.strftime("%Y-%m-%d")]
        ...
        for perfil in usuarios:
            ...
            # Sesiones del usuario en esos dos días exactos
            sesiones = SesionPomodoro.objects.filter(
                perfil=perfil,
                inicio__date__in=dias_validos
            ).order_by('-inicio')

            # Calcular promedio de elementos por wind
            winds = Wind.objects.filter(perfil=perfil)
            total_elementos = 0
            for w in winds:
                total_elementos += w.tareas.count() + w.notas.count()
            ...
            fila = [
                ...
                str(Wind.objects.filter(perfil=perfil).count()),
                str(promedio_elementos),
                str(Wind.objects.filter(perfil=perfil, focus_mode=True).count()),
                str(total),
        ...
        p.setFont("Helvetica", 9)
        p.drawString(2 * cm, y,
                     f"(Incluye solo sesiones de los días: {ayer.strftime('%d/%m/%Y')} y {hoy.strftime('%d/%m/%Y')})")
        y -= 1 * cm

        for perfil in usuarios:
            sesiones = SesionPomodoro.objects.filter(perfil=perfil, inicio__date=hoy).order_by('-inicio')
            ...
            if sesiones.exists():
                for s in sesiones:
                    ...
                    p.drawString(2 * cm, y, s.inicio.strftime("%d/%m/%Y %H:%M"))
                ...
                p.drawString(2 * cm, y, "No hay sesiones registradas para hoy.")
                y -= 0.5 * cm
...
class HistorialPomodoroAdminView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        perfil = get_object_or_404(Perfil, pk=kwargs['perfil_id'])
